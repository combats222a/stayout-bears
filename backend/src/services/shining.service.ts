import * as repo from '../repositories/shining.repository';
import type { AuthedRequest, ServiceResult } from '../types/http';

export async function getShining(req: AuthedRequest): Promise<ServiceResult> {
...
export async function setShining(req: AuthedRequest): Promise<ServiceResult> {
  if (!req.user.clan_id) return { status: 403, body: { error: 'Ты не в клане' } };
