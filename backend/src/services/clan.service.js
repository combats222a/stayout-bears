const repo = require('../repositories/clan.repository');
...
async function unbanMember(req) {
  if (!req.user.clan_id) return { status: 403, body: { error: 'Нет клана' } };
...
async function transferLeadership(req) {
  if (!req.user.clan_id) return { status: 403, body: { error: 'Нет клана' } };
...
async function refreshCode(req) {
  if (!req.user.clan_id) return { status: 403, body: { error: 'Нет клана' } };
...
module.exports = {
  createClan, joinClan, leaveClan, getMyClan, kickMember, banMember,
  unbanMember, transferLeadership, setDeputy, renameClan, refreshCode,
};
