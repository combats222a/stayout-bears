// Нулевая миграция — точное зеркало текущей backend/src/db/schema.js.
// Не меняет схему, только фиксирует её текущее состояние в системе миграций.
// Безопасна для уже проинициализированной БД: все statement'ы идемпотентны
// (CREATE TABLE IF NOT EXISTS / DO $$ ... IF NOT EXISTS ... $$).
//
// С этого момента дальнейшие изменения схемы — это НОВЫЕ файлы миграций
// (npm run migrate:create -- <name>), а не правки внутри schema.js.
...
exports.up = (pgm) => {
  pgm.sql(`
    ...
    -- Аномальные прорывы / Ледяная жара — одна запись на аккаунт (не на клан),
...
// down НЕ предназначен для продакшна без бэкапа — это грубый откат для локальной разработки.
exports.down = (pgm) => {
  pgm.sql(`
    DROP TABLE IF EXISTS user_anomaly;
    DROP TABLE IF EXISTS user_timers;
    DROP TABLE IF EXISTS clan_bans;
    DROP TABLE IF EXISTS password_reset_codes;
    DROP TABLE IF EXISTS loot_participants;
    DROP TABLE IF EXISTS shining;
    DROP TABLE IF EXISTS draugs;
