...
export async function initSchema(): Promise<void> {
  await pool.query(`
    ...
    -- Ограничение регистрации по IP (см. middleware/rateLimiter.ts):
    -- одна запись на IP, created_at обновляется при каждой успешной
    -- регистрации. Отдельная таблица, а не in-memory счётчик — бэкенд на
    -- Vercel serverless, память процесса между вызовами не сохраняется.
