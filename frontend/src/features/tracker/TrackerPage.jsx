...
export default function TrackerPage({ config, useStore, clan, isGuest, onLoginClick }) {
  ...
  const mergedItems = config.list.map(meta => {
  ...
  async function killItem(index, killedAt) {
    setError('');
    try {
