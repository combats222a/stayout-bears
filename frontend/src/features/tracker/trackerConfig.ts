...
// переиспользует их экспорты под общим интерфейсом. components/BearCard.jsx
// — мёртвый, нигде не импортируемый компонент с уже сломанными импортами
// из utils/bears (см. README.md → «Известные существующие проблемы») —
// сознательно не тронут при переводе на TypeScript.
