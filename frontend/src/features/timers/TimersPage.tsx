...
// Формат для <input type="datetime-local"> в ЛОКАЛЬНОМ времени пользователя
// (не UTC — toISOString() тут не подходит, он бы сдвинул время на часовой пояс).
...
// Компактный редактируемый "Интервал" прямо в колонке таблицы — три коротких
// поля дн./ч./мин. (старая механика инлайн-редактирования), но перекрашенные
// под новую цветовую схему таблицы (моно-шрифт, приглушённо-синий текст —
// как остальные значения в .td-interval). Правки не летят на сервер при
// каждом нажатии клавиши — небольшой debounce, а дальше если интервал
// уменьшили меньше текущего остатка, остаток подрезаем, чтобы прогресс-бар
// не зашкаливал.
function IntervalInlineEdit({ timer, onEdit }: { timer: UserTimer; onEdit: (id: number, changes: TimerEditChanges) => void }) {
  ...
  // Подхватываем интервал, если он поменялся снаружи (автообновление раз в
  // 30с, правка на другой вкладке) — но не пока пользователь сам сейчас печатает.
  useEffect(() => {
  ...
  function commit(nextDays: number, nextHours: number, nextMinutes: number) {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      if (newPeriodSeconds < 60) return; // интервал меньше минуты не отправляем
  ...
  return (
    <div className="interval-inline-edit" title="Интервал таймера">
      <input
        className="interval-inline-num" type="number" min="0" max="999"
        value={days} onFocus={e => e.target.select()}
        onChange={e => commit(Math.max(0, parseInt(e.target.value) || 0), hours, minutes)}
      />
      <span className="interval-inline-unit">д</span>
      <input
        className="interval-inline-num" type="number" min="0" max="23"
        value={hours} onFocus={e => e.target.select()}
        onChange={e => commit(days, Math.min(23, Math.max(0, parseInt(e.target.value) || 0)), minutes)}
      />
      <span className="interval-inline-unit">ч</span>
      <input
        className="interval-inline-num" type="number" min="0" max="59"
        value={minutes} onFocus={e => e.target.select()}
        onChange={e => commit(days, hours, Math.min(59, Math.max(0, parseInt(e.target.value) || 0)))}
      />
      <span className="interval-inline-unit">м</span>
    </div>
  );
}

// ── Модалка редактирования таймера (название + оставшееся время) ──
// Интервал больше здесь не редактируется — он снова правится прямо в
// таблице (см. IntervalInlineEdit), как и было в исходной механике.
function EditTimerModal({ timer, onCommit, onClose }: {
  timer: UserTimer; onCommit: (changes: TimerEditChanges) => void; onClose: () => void;
}) {
  const [name, setName] = useState(timer.name);

  // Оставшееся время — отдельное поле, не связанное с интервалом.
  ...
  // момент ОТКРЫТИЯ модалки, и при сохранении таймер откатился бы назад на
  // секунды/минуты, утёкшие пока модалка была открыта.
...
// ── Модалка редактирования "Доступен в" — колонка тоже стала кликабельной
// (как "Время смерти" у медведей): можно вписать точную дату-время, когда
// таймер должен стать доступен, а не только поправлять "Осталось" в минутах.
// Остаток пересчитывается от введённого времени, поэтому бар и обратный
// отсчёт в "Осталось" автоматически синхронизируются с новым значением.
function AvailableAtModal({ timer, onCommit, onClose }: {
  timer: UserTimer; onCommit: (changes: TimerEditChanges) => void; onClose: () => void;
}) {
  const initialDate = getForecast(timer) || new Date(Date.now() + timer.period_seconds * 1000);
  const [value, setValue] = useState(() => toLocalInputValue(initialDate));
  const [error, setError] = useState('');

  function handleSubmit() {
    if (!value) { setError('Укажите дату и время'); return; }
    const target = new Date(value);
    if (isNaN(target.getTime())) { setError('Некорректное время'); return; }
    const remainingSeconds = Math.round((target.getTime() - Date.now()) / 1000);
    const clamped = Math.min(Math.max(0, remainingSeconds), timer.period_seconds);
    onCommit({ remaining_seconds: clamped });
    onClose();
  }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box edit-timer-modal" onClick={e => e.stopPropagation()}>
        <div className="modal-title">
          <div className="modal-title-main">
            <span className="modal-title-icon-box"><EditPencilIcon /></span>
            <span className="modal-title-text">Когда таймер станет доступен</span>
          </div>
          <button className="modal-close-btn" onClick={onClose} aria-label="Закрыть">✕</button>
        </div>
        <div className="modal-body">
          ...
          <input
            ...
            autoFocus
          />
          <div className="modal-hint">"Осталось" и полоса прогресса пересчитаются автоматически.</div>
...
function TimerRow({
  ...
  const [showAvailableEdit, setShowAvailableEdit] = useState(false);
  ...
  return (
    <>
      ...
      >
        ...
        <td className="td-interval" data-label="Интервал">
          <IntervalInlineEdit timer={timer} onEdit={onEdit} />
        </td>
        ...
        <td className="td-clock" data-label="Доступен в">
          <span
            className={`clock-editable${isEmpty ? ' clock-empty' : ''}`}
            title="Нажми чтобы поменять время — «Осталось» пересчитается автоматически"
            onClick={() => setShowAvailableEdit(true)}
          >
            {isEmpty
              ? '--:--'
              : isExpired
      ...
      {showAvailableEdit && (
        <AvailableAtModal
          timer={timer}
          onCommit={changes => onEdit(timer.id, changes)}
          onClose={() => setShowAvailableEdit(false)}
        />
      )}
    </>
  );
