...
export interface InfoSpoilerBlock {
  heading: string;
  // Обычно массив строк, но CapturesPage.tsx подставляет туда готовые
  // <span> с цветными звёздочками (см. capturesSpoiler) — поэтому массив
  // допускает произвольный ReactNode, а не только строки.
  body: string | ReactNode[];
}

export interface InfoSpoilerContent {
  icon?: string;
  title: string;
  blocks: InfoSpoilerBlock[];
}
...
function writeStoredOpen(storageKey: string | undefined, value: boolean): void {
...
function BlockBody({ body }: { body: string | ReactNode[] }) {
...
interface InfoSpoilerProps extends InfoSpoilerContent {
  defaultOpen?: boolean;
  storageKey?: string;
}

export default function InfoSpoiler({ icon, title, blocks, defaultOpen = false, storageKey }: InfoSpoilerProps) {
