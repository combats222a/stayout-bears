...
function onlyDigits(str: string, maxLen: number): string {
  return (str || '').replace(/\D/g, '').slice(0, maxLen);
}

// "233523" (segments=3) -> "23:35:23"
// "0913"   (segments=2) -> "09:13"
function formatDigits(digits: string, segments: number): string {
  const parts: string[] = [];
...
function digitIndexToFormattedPos(digitsLen: number, digitIndex: number, segments: number): number {
  let extra = 0;
...
export default function MaskedTimeInput({
  ...
  function placeCaret(newDigits: string, newDigitIndex: number) {
  ...
  function commit(newDigits: string, newDigitIndex: number) {
    setDigits(newDigits);
...
export function digitsToTimeStr(digits: string, segments: number): string {
...
export function isDigitsComplete(digits: string, segments: number): boolean {
