...
// ── Локации ──────────────────────────────────────────────────────
export interface ShiningLocation {
  id: string;
  label: string;
  offset: number;
  name: string;
 ...
 */
export function parseGameTimeInput(gameTimeStr: string): string | null {
 ...
 */
export function getLiveGameTime(anchorGameTimeStr: string | null | undefined, anchorRealMs: number, slotOffsetHours: number, nowMs: number = Date.now()): string {
  // Z в минутах
 ...
 */
export function isShiningActive(liveGameTimeStr: string): boolean {
 ...
 */
export function getSlotRealStartTime(anchorGameTimeStr: string | null | undefined, anchorRealMs: number, slotOffsetHours: number, nowMs: number = Date.now()): number {
 ...
 */
export function getShiningCards(anchorGameTimeStr: string | null | undefined, anchorRealMs: number, nowMs: number = Date.now()): ShiningCard[] {
 ...
 */
export function computeShiningSlots(anchorGameTimeStr: string | null | undefined, anchorRealMs: number, nowMs: number): ShiningSlot[] {
  const parts = (anchorGameTimeStr || '00:00').trim().split(':').map(Number);
  const [gh = 0, gm = 0] = parts;
  const Z_minutes = gh * 60 + gm;
...
// ── Форматирование ────────────────────────────────────────────────
export function formatRealTime(ms: number | null | undefined): string {
  if (!ms && ms !== 0) return '--:--:--';
...
export function getSlotGameTime(baseGameTimeStr: string, slotIndex: number): string {
  return getLiveGameTime(baseGameTimeStr, Date.now(), slotIndex * 6, Date.now());
}

export interface UpcomingShiningSlot {
  realAt: number;
  index: number;
}

export function getUpcomingShiningSlots(anchorIso: string, refTimeMs: number = Date.now()): UpcomingShiningSlot[] {
