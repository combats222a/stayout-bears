...
// Любой объект с этими полями — bears приходят и с бэкенда (BearWithKiller),
// и как элементы стора; тип оставлен минимальным по той же причине.
interface TimedItem {
...
export function getTimeLeftMs(bear: TimedItem): number {
...
export function formatElapsed(ts: string | null | undefined): string {
  if (!ts) return '--:--:--';
...
export function getProgress(bear: TimedItem): number {
 ...
 */
export function parseLocalTimeInput(timeStr: string): string | null {
 ...
 */
export function spawnAtFromKilledAt(killedAtIso: string): string {
 ...
 */
export function killedAtFromSpawnAt(spawnAtIso: string): string {
 ...
 */
export function elapsedFromKilledAt(killedAtIso: string): string {
