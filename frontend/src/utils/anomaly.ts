...
//     локация 'gmt0' из shining.ts — выбор недоступен, всегда она)
...
export const ANOMALY_CYCLES: AnomalyCycle[] = [
 ...
 */
export function computeAnomalySlots(anchorGameTimeStr: string | null | undefined, anchorRealMs: number, nowMs: number = Date.now()): AnomalySlot[] {
  ...
  const windows: Array<{ warnStartAbs: number; burnStartAbs: number; burnEndAbs: number }> = [];
...
export function getNearestAnomalySlot(anchorGameTimeStr: string | null | undefined, anchorRealMs: number | null | undefined, nowMs: number = Date.now()): AnomalySlot | null {
