...
import type { BearWithKiller, DraugWithKiller, UserTimer } from '../types/entities';

interface ShiningWatchData {
  gameTimeStr?: string;
  anchorRealMs?: number;
}

interface AnomalyWatchData {
  gameTimeStr?: string;
  anchorRealMs?: number;
}

interface UseGlobalSoundWatcherArgs {
  token: string | null | undefined;
  bears: BearWithKiller[] | undefined;
  draugs: DraugWithKiller[] | undefined;
  shiningData: ShiningWatchData | null | undefined;
  anomalyData: AnomalyWatchData | null | undefined;
}

function getTimerRemainingSeconds(timer: UserTimer): number | null {
 ...
 * везде.
 */
export function useGlobalSoundWatcher({ token, bears, draugs, shiningData, anomalyData }: UseGlobalSoundWatcherArgs): void {
  const bearStateRef = useRef<Record<number, { key: string; warned: boolean }>>({});
  const draugStateRef = useRef<Record<number, { key: string; warned: boolean }>>({});
  const shiningStateRef = useRef<{ key: string; burning: boolean } | null>(null);
  const anomalyStateRef = useRef<{ key: string; warning: boolean } | null>(null); // независим от клана/входа
  const timersListRef = useRef<UserTimer[]>([]); // последний загруженный список таймеров
  const timerStateRef = useRef<Record<number, { key: string; wasExpired: boolean }>>({});
  const captureStateRef = useRef<Record<string, { key: number; isActive: boolean }>>({});
