...
// Браузеры блокируют AudioContext до первого user gesture.
// Мы создаём один контекст и разблокируем его явно при первом
// взаимодействии пользователя (кнопка «Установить время»).
...
/**
 * Вызвать при user gesture (клик «Сохранить» в модалке).
 * Разблокирует AudioContext, чтобы последующие автозвуки работали.
 */
export function unlockAudio() {
  try {
    ...
    // Тихий импульс — гарантирует разблокировку в Safari
...
export function playSpawnSound() {
  try {
    const ctx = getCtx();
    if (ctx.state === 'suspended') return; // ещё не разблокирован
