...
# 🏁 Весь проект переведён на TypeScript (полная типизация, не только каркас)

Предыдущий раздел выше касался только структуры (слои, `allowJs: true`,
старые `.js`/`.jsx` продолжали работать нетронутыми). Этот шаг —
собственно перевод: **все** файлы бэкенда и фронтенда стали `.ts`/`.tsx`
с полной типизацией, `allowJs` для основного кода больше не нужен.

**Backend** — 46 файлов, слоями снизу вверх: `db/` → `repositories/` →
`services/` → `controllers/` → `middleware/` → `sockets/` → `routes/` →
`index.ts`. Добавлены типы-модели в `types/entities.ts` (зеркалят
`db/schema.ts` и реальные проекции репозиториев), `types/http.ts`
(`AuthedRequest`, `ServiceResult`), `types/socket.d.ts` (аугментация
`Socket.user`). `types/express.d.ts` переписан на импорт `AuthUser` из
`entities.ts` вместо дублирования.

Раньше TS был только для тайпчека (`tsc --noEmit`), рантайм — на `.js`
напрямую (`node src/index.js`). Теперь рантайму нужен реальный TS-раннер:
добавлен `tsx` для `npm run dev` (`tsx watch src/index.ts`) и
`tsconfig.build.json` (наследует основной конфиг, `noEmit: false`,
`outDir: dist`) для `npm run build` + `npm run start` (`node dist/index.js`).
`db/migrations/0001_init.js` сознательно оставлен `.js` — конвенция
`node-pg-migrate` (`migrate:create -- -j js`), не часть рантайм-графа
приложения.

**Frontend** — весь `src/`, включая `App.jsx` → `App.tsx` и
`main.jsx` → `main.tsx`. Добавлен `vite-env.d.ts` (стандартный файл
шаблона Vite, которого не было — без него `import.meta.env` не
типизируется). `types/entities.ts` дополнен реальными формами ответов
API (`BearWithKiller`/`DraugWithKiller` с `killer_nick`, `ClanFull`,
`AdminUserSummary`, и т.д. — раньше файл был типами-заготовками без
единого фактического использования). Value-сторы `clan/store.ts`,
`shining/store.ts`, `anomaly/store.ts` — `createValueStore(null)` без
явного дженерика TypeScript выводил как буквальный тип `null`
(соответственно `never[]` для `[]`), из-за чего запись в стор реальных
данных стала бы ошибкой типов; исправлено явными generic-параметрами
(`createValueStore<Clan | null>(null)` и т.п.).

**Два сознательных исключения, не переведены:**
- `components/BearCard.jsx` — мёртвый компонент (нигде не импортируется,
  см. список известных проблем выше), содержит несуществующие импорты
  `getTimeLeft`/`formatTime` из `utils/bears`. В JS это молча становится
  `undefined` и не мешает, пока компонент не вызван. TypeScript, в
  отличие от JS, не даёт скомпилироваться при импорте несуществующих
  экспортов — типизировать файл с сохранением текущего (нерабочего, но
  недостижимого) поведения невозможно без исправления самого импорта,
  что выходит за рамки перевода на TS.
- `features/admin/AdminPage.jsx` (→ `.tsx`) содержал тот же несуществующий
  импорт `formatTime`/`getTimeLeft`, но **неиспользуемый** — в отличие от
  `BearCard`, ни разу не вызывается внутри файла. Удаление мёртвого
  импорта без единого места использования не меняет поведение ни при
  каких условиях, поэтому убран при переводе (это не то же самое, что
  исправление бага — здесь не менялось ничего, что реально исполнялось).
  Захардкоженное «9 медведей» (должно быть 11) в этом же файле — реальный
  баг с видимым эффектом, не трогался, перенесён как есть.

**Проверено:** `tsc --noEmit` — 0 ошибок отдельно на бэкенде и
фронтенде. `npm run build` чист на обоих (бэкенд — `tsc` эмит в `dist/`,
фронтенд — `vite build`, 127 модулей). Рантайм-smoke-тест бэкенда
(`node dist/index.js` и `tsx src/index.ts` с фиктивным `DATABASE_URL`) —
оба доходят ровно до попытки подключения к БД и падают там же, где упал
бы оригинальный `.js`-сервер без доступа к Postgres, — значит все
импорты, экспорты и типы на месте, ничего не сломано на уровне сборки
модулей. Поведение не менялось нигде, кроме двух исключений выше.

## Сделано: оптимизация — code-splitting (frontend), индексы и gzip (backend)

**Frontend.** Все 15 feature-страниц (`BearsPage`, `DraugsPage`,
`ShiningPage`, `ClanPage`, `AdminPage`, `ProfilePage`, `HeartsPage`,
`TimersPage`, `TimeCalcPage`, `LevelPage`, `FaqPage`, `CapturesPage`,
`AchievementsPage`, `AnomalyPage`) в `app/App.tsx` переведены с обычного
импорта на `React.lazy`, при этом рендерится всегда ровно одна — раньше
все 15 (включая Admin и Achievements на 43 KB) грузились в общий бандл ещё
до первого рендера. `Header`/`AuthPage`/`PublicLandingPage` остались
обычным импортом — это то, что видит гость первым делом, лишний сетевой
запрос там не нужен. `PromoPage` тоже оставлен обычным импортом: он и так
попадает в главный чанк через статический импорт внутри
`PublicLandingPage`, `lazy()` для него ничего не даёт, только плодит
предупреждение сборки Vite — проверено эмпирически, откачено обратно.
Четыре места рендера (`/level` для гостя, `/faq` для гостя, guest-preview
`<main>`, основной `<main>`) обёрнуты в `<Suspense fallback={...}>` с тем
же визуальным паттерном `.page`/`.loading`, что уже использовался внутри
`AdminPage`/`TimersPage` во время их собственной загрузки данных — при
переключении раздела не появляется ранее не виданный вид «загрузки».

**Проверено:** `tsc --noEmit` и `vite build` — без новых ошибок. Главный
чанк уменьшился с 403.13 KB (129.31 KB gzip) до 254.96 KB (85.17 KB
gzip); 15 страниц стали отдельными чанками (0.37–43.25 KB каждый),
подгружаются по факту захода в раздел и кэшируются браузером на будущее.
Хэш сборки не совпадает с предыдущей (ожидаемо — как и на шаге
App.jsx → app/, здесь реально меняется механизм загрузки модулей, а не
только путь файла); поведение проверено построчно — набор условий
`page === '...'` и передаваемые пропсы не менялись, менялся только способ
импорта компонента.

**Backend — индексы.** Postgres не создаёт индекс на колонку внешнего
ключа автоматически — только на PRIMARY KEY/UNIQUE. Три колонки без
индекса участвовали в `WHERE` самых частых запросов и сканировались
последовательно: `users.clan_id` (`getMembers` — при входе, при каждом
`socket clan:update`, и раз в 30с polling с каждого открытого клиента),
`loot_participants.clan_id` (список сердец/шкур), `user_timers.user_id`
(список таймеров). Добавлены `CREATE INDEX IF NOT EXISTS` — в
`schema.ts` (выполняется при каждом старте сервера) и зеркалом в новой
миграции `0002_add_performance_indexes.js`, по той же схеме дублирования,
что и `0001_init.js` ↔ `schema.ts`.

**Backend — gzip.** Подключён `compression` (Express-мидлварь) в
`index.ts`, до `express.json()`. Сжимает все ответы прозрачно для
клиентов — `fetch`/`socket.io-client` уже шлют `Accept-Encoding: gzip`
по умолчанию. Заметнее всего на `/clans/me` (bears+draugs+members+bans
одним ответом), который гоняется каждые 30с с каждого открытого клиента.

**Проверено:** `tsc --noEmit` и `tsc -p tsconfig.build.json` — без новых
ошибок. `node --check dist/index.js` — синтаксис скомпилированного
вывода валиден. Все три изменения аддитивны (новый индекс/новая
мидлварь), ни один существующий SQL-запрос или JSON-ответ не менялся —
`compression` не трогает тело ответа, только его транспортное кодирование,
а индекс не меняет результат `WHERE`, только его стоимость.

**Известное, не относящееся к этому шагу:** в загруженном архиве нет ни
одного dotfile (`.eslintrc`, `.gitignore` и т.п.) — судя по всему, экспорт
в zip не включает файлы, начинающиеся с точки. Из-за этого `npm run lint`
не может найти конфиг ESLint в песочнице; `typecheck`/`build` это не
касается, оба прошли зелёными.

## Сделано: распараллеливание независимых запросов (Promise.all)

При обходе сервисов на предмет ещё одной категории «не меняет результат,
только ускоряет» нашлось два места с несколькими независимыми
`await pool.query(...)` подряд — каждый следующий ждал завершения
предыдущего, хотя ни один не зависит от результата другого (все читают по
одному и тому же `clanId`, а не по данным из соседнего запроса).

**`clan.repository.ts → getClanFull`** — самый частый запрос в
приложении (`loadClan`: при входе, при каждом `socket clan:update`, и раз
в 30с polling с каждого открытого клиента). Было 5 последовательных
`await`: клан → участники → bears → draugs → баны. Теперь все 5 — в одном
`Promise.all`. Проверка `if (!clan) return null` осталась ПОСЛЕ
`Promise.all` — если у пользователя завис устаревший `clan_id`
несуществующего клана (в норме такого быть не должно, но на всякий
случай), 4 лишних запроса с пустым результатом выполнятся вхолостую, но
ответ вызывающему коду будет тем же самым `null`, что и раньше. В обычном
случае (клан существует) — 5 круговых обращений к БД одно за другим стали
одним параллельным заходом.

**`admin.repository.ts → getAllClansOverview`** — тот же приём для 3
независимых full-table select'ов (clans/users/bears) на странице
суперадминки. Трафик там низкий, но правка той же формы и той же
безопасности, так что применена заодно.

**Проверено:** `tsc --noEmit` и `tsc -p tsconfig.build.json` — без новых
ошибок (после переустановки `node_modules`, который сам же случайно
удалил перед сборкой предыдущего архива — `npx tsc` подхватил чужую
глобальную версию TypeScript и выдал два нерелевантных предупреждения
про deprecated-опции конфига, которых с версией из `package.json`
(`^5.4.5`) нет). `node --check` на всех трёх скомпилированных файлах —
синтаксис валиден. Порядок деструктуризации в обоих `Promise.all`
построчно сверен с порядком элементов массива — типовая ошибка в этом
месте была бы молча перепутать поля местами, её нет.
